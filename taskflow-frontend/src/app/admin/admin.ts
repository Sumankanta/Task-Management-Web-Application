...
export class AdminComponent implements OnInit {
  ...
  // Edit
  showEditModal = false;
  editUserData: any = { id: null, fullName: '', email: '' };
  ...
  this.adminService.updateStatus(user.id, newStatus).subscribe({
    next: (updated: any) => {
      // Use the value from the server response, not the local guess
      const u = this.users.find(x => x.id === user.id);
      if (u) u.isActive = updated.isActive;
      this.toast.show(`User ${updated.isActive ? 'activated' : 'deactivated'}`, 'success');
    },
    error: () => this.toast.show('Failed to update status', 'error')
  });
}
  ...
  openEdit(user: any) {
    this.editUserData = { id: user.id, fullName: user.fullName, email: user.email };
    this.showEditModal = true;
  }

  saveEdit() {
    const u = this.users.find(x => x.id === this.editUserData.id);
    if (u) {
      u.fullName = this.editUserData.fullName;
      u.email    = this.editUserData.email;
    }
  ...
  getAvatarColor(name: string): string {
    const colors = [
      '#58a6ff', '#f85149', '#bc8cff', '#3fb950',
      '#e3b341', '#79c0ff', '#ffa657', '#56d364'
    ];
    let hash = 0;
